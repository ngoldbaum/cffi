...
* Fixed races in callback registration and cleanup, function-pointer caching,
  and embedding initialization.  Atomic publication helpers are now shared by
  GIL-enabled and free-threaded builds.
* Type realization now preserves canonical type identities and completed
  layouts when garbage collection or a field conversion reenters CFFI.
  Failed layout construction leaves the type available for a later retry.
* Reduced reference-count overhead for ``extern "Python"`` callbacks.
* Building ``_cffi_backend`` or generated embedding code with a non-MSVC
  compiler now requires GCC-compatible ``__atomic`` builtins (GCC 4.7 or newer,
  or a compatible compiler such as Clang).
