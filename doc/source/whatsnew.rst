...
* Fixed data races found with ThreadSanitizer on the free-threaded build in
  ``extern "Python"`` dispatch, ``ffi.addressof(lib, ...)`` caching, lazy
  struct size reads, callbacks from native threads, and embedding startup.
  Building ``_cffi_backend`` or generated embedding code with a non-MSVC
  compiler now requires GCC-compatible ``__atomic`` builtins (GCC 4.7 or
  newer, or Clang).
