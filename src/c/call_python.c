...
static int _ffi_def_extern_publish(struct _cffi_externpy_s *externpy,
                                   PyObject *infotuple)
{
    /* called with CFFI_LOCK held; consumes a reference to infotuple */
    PyObject *interpstate_dict, *interpstate_key, *old1;
    int err;
...
{
    const char *s;
    PyObject *error, *onerror, *infotuple;
    int index, err;
    ...
    CFFI_LOCK();
    err = _ffi_def_extern_publish(externpy, infotuple);
    CFFI_UNLOCK();
    if (err < 0)
        return NULL;
...
static void cffi_call_python(struct _cffi_externpy_s *externpy, char *args)
{
    ...
    */
    ...
    /* Keep this read barrier for modules generated by older _embedding.h
       versions, paired with their write_barrier().  Without it, we can
